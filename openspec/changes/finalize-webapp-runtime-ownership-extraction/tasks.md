...
- [ ] 2.1 Create a focused client-side runtime owner for skill event timeline and batch consumption with explicit dependency inputs from App.
- [ ] 2.2 Move timeline scheduling, scheduled event consumption, immediate event batching, active damage-zone tick routing, projectile follow-up suppression, VFX queue updates, status routing, forced movement routing, floating text routing, and damage-event routing into the event-consumption owner without changing behavior.
