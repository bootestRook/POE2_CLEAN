...
- 12.3 smoke tests now collect WebApp source files with relative paths and fail if any extracted `.ts`/`.tsx` module imports from `webapp/App.tsx`; `webapp/main.tsx` is the only allowed App importer because it is the React bootstrap entry.
- 12.4 smoke tests now fail on new WebApp backend/server gameplay coupling tokens (`fetch`, API paths, server runtimes, runtime-skill request shims), skill-editor acceptance paths (`/skill-editor`, query flags, port `8765`, and `dist-skill-editor`), and App gameplay-runtime ownership duplicated inside extracted presentation/state modules.
- 12.5 smoke tests now read `docs/webapp-module-boundaries.md` and `docs/webapp-app-decomposition-map.md` directly and require the planning rule, missing-boundary rule, App allowed edits, target owner folders, completion definition, and `run.bat` verification guidance to remain AI-visible.
- 12.6 each 12.x test batch was verified with `cmd /c npm run build`, `npm test`, and `openspec validate finalize-webapp-app-architecture-boundaries --strict`. The repository's accepted full WebApp test command remains `npm test`, which runs `node webapp/smoke-test.mjs`.

## Final Frontend Verification

- 13.1-13.3 passed `cmd /c npm run build`, `npm test`, and direct `node webapp/smoke-test.mjs`.
- 13.4-13.7 used the project `run.bat` flow at `http://127.0.0.1:8766/`, with logs stored in `artifacts/logs/runbat-webapp-final-out.log` and `artifacts/logs/runbat-webapp-final-err.log`. Final screenshots are under `artifacts/screenshots/`: `final-13e-title-screen.png`, `final-13e-save-selection.png`, `final-13e-rest-area.png`, `final-13e-inventory-overlay.png`, `final-13e-stash-surface.png`, `final-13e-map-selection.png`, `final-13e-playable-battle.png`, and `final-13e-battle-pause-overlay.png`.
- 13.5 observed the actual playable WebApp title/save/rest flow, inventory overlay, stash interaction, map selection, playable battle map, and battle pause overlay. The final browser DOM contained one `.battle-pause-overlay`, no map selection panel, two battle canvases, and combat text showing the map run had started. The battle screenshot shows the playable map after entering the selected map; the pause screenshot shows the pause menu over the playable battle surface.
- 13.8 root artifact scan found no root-level `.png`, `.jpg`, `.jpeg`, `.webp`, `.log`, `.trace`, or `.har` files. 13.9 verification did not navigate to `/skill-editor`, use skill-editor query flags, use port `8765`, or use `dist-skill-editor`; matching strings found by `rg` were only in smoke-test forbidden-token checks and OpenSpec documentation.

## Completion Review

- 14.1 final App review: `webapp/App.tsx` now imports focused owners for inventory presentation, layout shell/title presentation, monster skill event builders, player damage formulas, frontend state/drop helpers, stash helpers, and save storage helpers. Remaining App ownership is top-level mode routing, launch/bootstrap flags, cross-domain React state/ref initialization, callback wiring, runtime queues, battle event consumption, save/rest/map orchestration, and intentional adapters between focused modules.
- 14.2 large render-only inventory/equipment/stash/board presentation no longer lives inline in App. It is owned by `webapp/components/inventory/InventoryOverlay.tsx`, `EquipmentPanel.tsx`, `InventoryBagPanel.tsx`, `InventorySkillBoardPanel.tsx`, and `stashState.ts`; layout/title/shell overlays are owned by `webapp/components/layout/EntryTitleScreen.tsx`, `AppTopHud.tsx`, and `GameShellOverlays.tsx`.
- 14.3 monster skill pure helpers and payload builders are searchable in focused runtime modules: `webapp/runtime/monsterSkillPresentation.ts` and `webapp/runtime/monsterSkillEventBuilder.ts`.
- 14.4 player damage formulas are covered in `webapp/runtime/playerDamageRuntime.ts` smoke execution, while save/state/drop/stash storage semantics are checked from `frontendAppState.ts`, `frontendDropState.ts`, `frontendSaveStorage.ts`, and `stashState.ts`. Storage keys and save version were not changed.
- 14.5 future WebApp placement guidance is AI-visible in `docs/webapp-module-boundaries.md` and `docs/webapp-app-decomposition-map.md`, and `webapp/smoke-test.mjs` now fails if those documents lose the final owner map.
- 14.6 completion checks found no new backend coupling, server runtime behavior, duplicate extracted gameplay runtime, save-schema change, storage-key change, CSS redesign, copy change, gameplay balance change, unrelated refactor, root-level artifact, or skill-editor acceptance path. Smoke tests now guard those constraints directly.
- 14.7 intentionally remaining App-owned runtime orchestration includes runtime refs (`activeDamageZones`, `pendingBossDamageZoneHits`, player/enemy refs), skill event timeline/batch consumption, damage batch application, repeated monster zone scheduling, map-entry orchestration, save-slot flow wiring, and disabled backend-state shim behavior. These were not moved because the change was an App architecture boundary split, not a gameplay runtime rewrite.
- 14.8 final `openspec validate finalize-webapp-app-architecture-boundaries --strict` passed.
