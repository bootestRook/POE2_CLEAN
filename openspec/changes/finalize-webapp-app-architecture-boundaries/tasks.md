...
- [x] 4.4 Keep `GemOrb`, `GemTooltipOverlay`, `FloatingGemView`, `StashPanel`, `BagGrid`, `EquipmentItemCell`, `EquipmentEmptyCell`, `BoardCell`, `SupportLines`, and `SupportPreviewLines` behavior identical after extraction.
...
- [x] 5.2 Keep subpanel props explicit and avoid creating a new inventory state store, board state store, or drag/drop context in this change.
- [x] 5.3 Preserve all existing slot arrays, item lookup maps, hover state, floating item state, tooltip triggers, render callbacks, and placement callbacks.
- [x] 5.4 Confirm `webapp/App.tsx` no longer owns large inventory/equipment/stash/board JSX after this batch.
- [x] 5.5 Run focused checks and visually verify inventory, stash, equipment, and board rendering in the playable WebApp.
...
- [x] 6.3 Preserve every existing numeric formula, special skill-id case, VFX key string, pattern name, default value, and return shape.
...
- [x] 7.8 Run build/test checks and visually verify representative monster skill projectiles/zones/arcs in the actual playable WebApp, with screenshots under `artifacts/screenshots/`.
...
- [x] 8.2 Create `webapp/runtime/playerDamageRuntime.ts` only for pure mitigation/resource helpers that can move without changing state ownership.
...
- [x] 12.6 Run the relevant focused tests after each batch and the full test suite before completion.
