...
- [ ] 3.1 Extract rest-area presentation such as `RestAreaScene` and `RestAreaMapInteractableLayer` into `webapp/components/rest-area/`, preserving props, class names, text, and DOM order.
- [ ] 3.2 Extract save, map-selection, stash, and GM/tool panels only where they can remain prop-driven and not own App gameplay or save state.
