...
- Skill event consumption and runtime queues: `webapp/runtime/skillEventConsumerRuntime.ts` now owns `consumeSkillEventTimeline`, `consumeImmediateSkillEvents`, scheduled skill events, `consumeSkillEvent`, `consumeSkillEventBatch`, active damage-zone tick queues, projectile impact queues, and damage event routing; `webapp/App.tsx` wires refs, setters, and callbacks into that owner.
- Damage application and status/resource helpers: `webapp/runtime/damageApplicationRuntime.ts` now owns `applyDamageEventBatch`, enemy damage projection, HP/energy-shield mutation, kill detection, player on-hit recovery, enemy retention, combat logs, drops, and recursive on-kill event routing; formula helpers remain in `webapp/runtime/enemyDamageRuntime.ts`, and `webapp/App.tsx` wires refs, setters, and callbacks into the owner.
...
## Final Battle Loop Assessment

After extracting skill event consumption and damage application, `stepGame` is still intentionally App-owned orchestration. Its current body crosses elapsed-time state, war intent, player movement and recovery, drop pickup, boss portal use, minimap reveal, authored/procedural spawning, monster AI movement, monster skill runtime, player continuous/channel skill execution, projectile impact processing, boss projectile and damage-zone hit processing, active damage-zone ticks, scheduled skill events, pause/failure flow, and battle result counters. Moving it now would require either a very wide dependency bag or additional unrelated feature ownership moves, so this pass stops with `stepGame` documented as deferred orchestration rather than extracting a broad battle-loop hook.
